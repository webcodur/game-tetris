...
const Next = ({ nextTetrominos, $useBackgroundImage }) => (
  <StyledNextWrapper>
    ...
    {nextTetrominos.map((tetromino, index) => (
      <StyledNext key={index} width={tetromino[0].length} height={tetromino.length}>
        {tetromino.map((row, y) => row.map((cell, x) => <Cell key={x} type={cell} $useBackgroundImage={$useBackgroundImage} />))}
      </StyledNext>
    ))}
  </StyledNextWrapper>
);
